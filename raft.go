...
func newRaftNode(id int, peers []string, proposeC <-chan string) (<-chan *string, <-chan error) {
	rc := &raftNode{
		...
		id:          id,
		peers:       peers,
		waldir:      fmt.Sprintf("raftsql-%d", id),
...
func (rc *raftNode) openWAL() *wal.WAL {
	if wal.Exist(rc.waldir) == false {
		if err := os.Mkdir(rc.waldir, 0750); err != nil {
			log.Fatalf("raftsql: cannot create dir for wal (%v)", err)
		}

		w, err := wal.Create(rc.waldir, nil)
		if err != nil {
			log.Fatalf("raftsql: create wal error (%v)", err)
		}
		w.Close()
	}
	...
	if err != nil {
		log.Fatalf("raftsql: error loading wal (%v)", err)
	}
...
func (rc *raftNode) replayWAL() *wal.WAL {
	...
	if err != nil {
		log.Fatalf("raftsql: failed to read WAL (%v)", err)
	}
...
func (rc *raftNode) serveRaft() {
	...
	if err != nil {
		log.Fatalf("raftsql: Failed parsing URL (%v)", err)
	}

	ln, err := newStoppableListener(url.Host, rc.httpstopc)
	if err != nil {
		log.Fatalf("raftsql: Failed to listen rafthttp (%v)", err)
	}
	...
	case <-rc.httpstopc:
	default:
		log.Fatalf("raftsql: Failed to serve rafthttp (%v)", err)
	}
	close(rc.httpdonec)
}
